...
public class Test_Inference_Log_Pseudo_Likelihood {
...
//        System.out.println(instances.toSummaryString());
                ...
                String[] featureNames = new String[instances.numAttributes()];
                String[][] featureValueOptions = new String[instances.numAttributes()][];
                for (int i = 0; i < featureNames.length; i++) {
                    featureNames[i] = instances.attribute(i).name();
                    featureValueOptions[i] = new String[instances.attribute(i).numValues() + 1];//+1 for missing
                    for (int j = 0; j < featureValueOptions[i].length - 1; j++) {
                        featureValueOptions[i][j] = instances.attribute(i).value(j);
                    }
                    featureValueOptions[i][featureValueOptions[i].length - 1] = "missing";
                    System.out.println("Dom(" + featureNames[i] + ") = " + Arrays.toString(featureValueOptions[i]));
                }
...
//        System.out.println("Learning...");
                ...
                //Start to inference
                Inference inference = new Inference(bestModel, featureNames, featureValueOptions);
                //??what's this??
                ...
                List<String> variablesNamesList = Arrays.asList(featureNames);
                List<String[]> outComeList = Arrays.asList(featureValueOptions);
...
//        System.out.println(outComeList.size());
                ...
                for (int i = 0; i < instances.numInstances(); i++) {
                    ...
                    for (int featureIndex = 0; featureIndex < featureNames.length; featureIndex++) {
                        ...
                        Attribute att = instances.attribute(featureIndex);
                        String featureName = instances.attribute(featureIndex).name();
                        ...
                        for (int evidenceIndex = 0; evidenceIndex < featureNames.length; evidenceIndex++) {
                            if (evidenceIndex == featureIndex) {
                                if (evidenceIndex == featureNames.length - 1) {
                                    inference.recordEvidence();
                                }
                                continue;
                            }
                            //find the attribute class object by name
                            Attribute evidenceAtt = instances.attribute(evidenceIndex);
                            String evidenceName = instances.attribute(evidenceIndex).name();
                            //get the value of the attribute
                            String evidenceValue = oneRowOfData.stringValue(evidenceAtt);
                            //deal with missing value
                            if (evidenceValue.equals("?") || evidenceValue.equals("")) {
                                evidenceValue = "missing";
                            }

                            inference.addEvidence(evidenceName, evidenceValue);
                ...
                try (FileOutputStream outputStream = new FileOutputStream(new File("./chodalysis_result/" + datasetName + "-" + datatype + "-log_pseudolikelihood_result.xlsx"))) {
